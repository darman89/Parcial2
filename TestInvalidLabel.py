...
from DatabaseConnector import select_Apk_ActivitiesNames, insert_ApkProperties, insert_ActivityNotDefinedTest, \
    insert_InvalidActivityNameTest, select_Apk_ActivitiesLabels, insert_InvalidLabelTest
...
class AndroTest(auto.DirectoryAndroAnalysis):
    ...
    def analysis_apk(self, log, apkobj):
        ...
        activities_labels = select_Apk_ActivitiesLabels()
        mutant_activities_labels = list(apkobj.get_all_attribute_value("activity", "label"))

        # Verificando la cantidad de actividades de las APKs
        act_mismatch = len(activities_labels) == len(mutant_activities_labels)

        result = []

        if act_mismatch:
            for apkmut in mutant_activities_labels:
                if apkmut not in activities_labels:
                    result.append(apkmut)

        insert_InvalidLabelTest(os.path.basename(log.filename), len(result) == 0,
